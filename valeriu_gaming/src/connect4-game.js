...
class ConnectFour {
    ...
    startGame(msg) {
        ...
        if (!oppenent) return msg.channel.send(`**Cu cine vrei sa te joci Connect? Da tag cuiva cand folosesti comanda**`)
        ...
        const initial = new discord.MessageEmbed()
            .setTitle(`🔴 E randul lui ${msg.author.username}!`)
            .setDescription(initialState)
            .setFooter(`${challenger.username} vs ${oppenent.username}`)
        msg.channel.send({ embed: initial }).then(gameMessage => {
                ...
                if (user.id === gameData[player].member.id) {
                    ...
                    switch (reaction.emoji.name) {
                        case "1️⃣":
                            ...
                            if (openSpaces.length == 0) return msg.channel.send(`**${gameData[player].member}, coloana e deja ocupata**`).then(msg1 => msg1.delete({ timeout: 10000 }))
                    ...
                    if (tieCheck()) {
                        ...
                        gameMessage.edit({ embeds: [TieEmbed] });
                        setTimeout(() => { if (gameMessage) gameMessage.delete(); }, 10000);
                        return;
                    }
                    ...
                    for (const func of checks) {
                        ...
                        if (data) {
                            ...
                            const WinEmbed = new discord.MessageEmbed()
                                .setTitle(`${gameData[player].member.username} a castigat!`)
                                .setDescription(renderBoard(board))
                                .setFooter(`${challenger.username} vs ${oppenent.username}`)
                            gameCollector.stop(`${gameData[player].member.id} won`);
                            setTimeout(() => { if (gameMessage) gameMessage.delete(); }, 10000);
                            gameMessage.edit({ embeds: [WinEmbed] })
                            return;
                        }
                    }
                    ...
                    const newEmbed = new discord.MessageEmbed()
                        .setTitle(`${gameData[player].playerColor} -  E randul lui ${gameData[player].member.username}!`)
                        .setDescription(renderBoard(board))
                        .setFooter(`${challenger.username} vs ${oppenent.username}`)
